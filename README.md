...
```bash
calltrace sequence examples/python --entry UserService.login --depth 4
npx --no-install mmdc -i examples/python/.calltrace/UserService.login/sequence.mmd -o artifacts/example.svg
```
...
Windows 未激活环境时，将 `python` 换为 `.\.venv\Scripts\python.exe`。测试包含跨文件绑定、错误绑定防护、调用次序、递归、深度和确定性契约。pytest 将集成测试生成的图导出到 `artifacts/mermaid/`，随后 npm 命令用固定版本的 Mermaid 官方解析器校验语法；该命令在没有图时会失败，不会把空检查报告为成功。Node.js/npm 仅用于这一步开发验证，Python CLI 不依赖它们。

`.venv/`、`node_modules/`、`.calltrace/` 和 `artifacts/` 均是本地环境或生成产物，已列入忽略规则。测试通过表示覆盖的契约成立，不表示支持矩阵外的语义已得到证明。
